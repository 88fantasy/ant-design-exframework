...
const RouterMenu: React.FC<RouterMenuPropsType> = (props) => {
      ...
      {showEdit ? (
        <div className={`${prefixCls}-menuHeader`}>
          ...
          <div className={`${prefixCls}-buttonWrapper`}>
            <span
              className="save"
              onClick={() => onSave && onSave(checkedMenus)}
            >
              保存
            </span>
            <span className="cancel" onClick={() => onCancel && onCancel()}>
              取消
            </span>
          </div>
        </div>
            ...
            return (
              <div key={key} className={`${prefixCls}-menuItemWrapper`}>
                <div className={`${prefixCls}-menuItemHeader`}>
                  ...
                  <span className={`${prefixCls}-menuItemHeaderName`}>
                    {name}
                  </span>
                </div>
                {showEdit ? (
                  ...
                  >
                    {!hideChildrenInMenu && children.length ? (
                      children.map((childItem) => {
                        ...
                        return childName ? (
                          ...
                          >
                            {childName}
                            {childTarget ? <ShareAltOutlined /> : null}
                          </Checkbox>
  ...
  return (
    <Popover
      content={menuConetent}
      placement="bottom"
      overlayClassName={`${prefixCls}-disabledBox`}
    >
      {title} <DownOutlined />
    </Popover>
  );
